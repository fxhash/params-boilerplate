...
const InjectHead = require("../config/plugins/InjectHead")
...
;(async () => {
  ...
  const compiler = Webpack({
    ...
    stats: "errors-only",
    // add a plugin to inject location modifier into the head of document
    // plugins: [
    //   ...webpackConfig.plugins,
    //   new InjectHead({
    //     inject: `<script>window.location = localhost:${env.PORT_FXSTUDIO}</script>`
    //   })
    // ]
  });
